...
export class VoicevoxService {
  ...
  // C API関数
  private voicevox_onnxruntime_load_once: ((options: unknown, out: unknown) => number) | null = null;
  private voicevox_open_jtalk_rc_new: ((path: string, out: unknown) => number) | null = null;
  private voicevox_synthesizer_new: ((onnx: unknown, jtalk: unknown, options: unknown, out: unknown) => number) | null = null;
  private voicevox_voice_model_file_open: ((path: string, out: unknown) => number) | null = null;
  private voicevox_synthesizer_load_voice_model: ((synth: unknown, model: unknown) => number) | null = null;
  private voicevox_voice_model_file_delete: ((model: unknown) => void) | null = null;
  private voicevox_synthesizer_tts: ((synth: unknown, text: string, styleId: number, options: unknown, outLen: unknown, outWav: unknown) => number) | null = null;
  private voicevox_wav_free: ((wav: unknown) => void) | null = null;
  private voicevox_make_default_load_onnxruntime_options: (() => unknown) | null = null;
  private voicevox_make_default_initialize_options: (() => unknown) | null = null;
  ...
  async initialize(): Promise<void> {
    ...
    const dictPath = path.join(VOICEVOX_CORE_DIR, 'dict', 'open_jtalk_dic_utf_8-1.11');
    ...
    try {
      ...
      this.voicevox_onnxruntime_load_once = this.lib.func(
        ...
        'int32',
        [VoicevoxLoadOnnxruntimeOptions, koffi.out(koffi.pointer(VoicevoxOnnxruntime))]
      );
      ...
      this.voicevox_synthesizer_new = this.lib.func(
        ...
        'int32',
        [koffi.pointer(VoicevoxOnnxruntime), koffi.pointer(OpenJtalkRc), VoicevoxInitializeOptions, koffi.out(koffi.pointer(VoicevoxSynthesizer))]
      );
      ...
      this.voicevox_synthesizer_tts = this.lib.func(
        ...
        'int32',
        [koffi.pointer(VoicevoxSynthesizer), 'const char *', 'uint32', VoicevoxTtsOptions, koffi.out('uintptr_t *'), koffi.out('uint8_t **')]
      );

      this.voicevox_wav_free = this.lib.func(
        'voicevox_wav_free',
        'void',
        ['uint8_t *']
      );
      ...
      // ONNX Runtimeをロード
      const onnxruntimeOptions = this.voicevox_make_default_load_onnxruntime_options();
      const onnxruntimeOut: unknown[] = [null];
      const onnxResult = this.voicevox_onnxruntime_load_once(onnxruntimeOptions, onnxruntimeOut);
      if (onnxResult !== 0) {
      ...
      const jtalkResult = this.voicevox_open_jtalk_rc_new(dictPath, openJtalkOut);
      if (jtalkResult !== 0) {
      ...
      const synthResult = this.voicevox_synthesizer_new(this.onnxruntime, this.openJtalk, initOptions, synthesizerOut);
      if (synthResult !== 0) {
      ...
      if (existsSync(modelPath)) {
        ...
        const modelOpenResult = this.voicevox_voice_model_file_open(modelPath, modelOut);
        if (modelOpenResult === 0) {
          const loadResult = this.voicevox_synthesizer_load_voice_model(this.synthesizer, modelOut[0]);
          if (loadResult !== 0) {
   ...
   * @returns WAV形式の音声データ
   */
  async synthesis(text: string, speakerId: number = ZUNDAMON_STYLE_ID): Promise<Uint8Array> {
    if (!this.initialized || !this.synthesizer || !this.voicevox_synthesizer_tts || !this.voicevox_make_default_tts_options || !this.voicevox_wav_free) {
